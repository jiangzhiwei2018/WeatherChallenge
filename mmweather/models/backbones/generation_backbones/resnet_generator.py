...
from mmweather.models.common import (ResidualBlockWithDropout,
                                     generation_init_weights, UpSampleMixed)
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self,
                 ...
                 init_cfg=dict(type='normal', gain=0.02), upsample_type="interpolate"):
        super().__init__()
        ...
        for i in range(num_blocks):
            model += [
                ResidualBlockWithDropout(
                    ...
                    norm_cfg=norm_cfg,
                    act_cfg=act_cfg,
                    use_dropout=use_dropout, conv_cfg=conv_cfg
                )
            ]
        ...
        for i in range(num_down):
            ...
            model += [
                ConvModule(
                    ...
                    norm_cfg=norm_cfg) if upsample_type == "deconv" else
                nn.Sequential(
                    UpSampleMixed(base_channels * multiple, base_channels * multiple // 2, 2,
                                  upsample_kernel=3,
                                  upsample_type=upsample_type),
                    build_norm(norm_cfg, base_channels * multiple // 2, 2),
