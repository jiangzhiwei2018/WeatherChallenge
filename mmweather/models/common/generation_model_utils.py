...
from mmcv.cnn import ConvModule, kaiming_init, normal_init, xavier_init, build_activation_layer
from torch.nn import init
import torch.nn.functional as F
from mmweather.models.builder import build_norm
from mmweather.models.common.upsample import UpSampleMixed
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        use_bias = 'auto'  # if norm_cfg is None else norm_cfg['type'] == 'IN'
        kernel_size = 4
        stride = 2
        padding = 1
        if in_channels is None:
            in_channels = outer_channels
        down_conv_cfg = conv_cfg
        ...
        up = [
            ConvModule(
                ...
                act_cfg=up_act_cfg,
                order=('act', 'conv', 'norm')) if upsample_type == "deconv" else
            nn.Sequential(
                build_activation_layer(up_act_cfg),
                UpSampleMixed(up_in_channels, outer_channels, 2,
                              upsample_kernel=kernel_size,
                              upsample_type=upsample_type),
                build_norm(up_norm_cfg, outer_channels),
            ),
        ]
...
class ResidualBlockWithDropout(nn.Module):
    ...
    def __init__(self,
        ...
        # Only for IN, use bias since it does not have affine parameters.
        # use_bias = True if norm_cfg['type'] == 'IN' else 'auto'
        # use_bias = 'auto'
        use_bias = True if norm_cfg is None else norm_cfg['type'] == 'IN'
        ...
        block = [
            ConvModule(
                ...
                norm_cfg=norm_cfg,
                act_cfg=act_cfg,
                padding_mode=padding_mode, conv_cfg=conv_cfg)
        ]
