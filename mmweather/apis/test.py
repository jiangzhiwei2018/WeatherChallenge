...
def single_gpu_test(model,
    ...
    Args:
        ...
        iteration (None, int): Iteration number. It is used for the save image name.
            Default: None.
    ...
    summary_writer = kwargs.pop("summary_writer", None)
    if isinstance(test_val_steps, int):
        test_val_steps = [test_val_steps]
    for idx, data in enumerate(data_loader):
        if test_val_steps and (idx+1 not in test_val_steps):
            for key, val in data.items():
                if isinstance(val, torch.Tensor):
                    prog_bar.update(len(val))
                    break
            continue
        with torch.no_grad():
            result = model(
