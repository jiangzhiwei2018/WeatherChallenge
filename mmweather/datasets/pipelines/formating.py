...
class ImageToTensor:
    ...
    def __call__(self, results):
        ...
        for key in self.keys:
            ...
            results[key] = to_tensor(results[key].transpose(0, 3, 1, 2))
        return results
...
@PIPELINES.register_module()
class CopyData:
    def __init__(self, be_copy_key, copy_keys, del_org=True):
        """
        """
        assert isinstance(be_copy_key, str)
        assert isinstance(copy_keys, (list, tuple))
        for key in copy_keys:
            assert key != be_copy_key, f"be_copy_key should not in {copy_keys}"
        self.be_copy_key = be_copy_key
        self.copy_keys = copy_keys
        self.del_org = del_org
    ...
    def __call__(self, results):
        ...
        :return:
        """
        data = copy.deepcopy(results[self.be_copy_key])
        for key in self.copy_keys:
            results[key] = copy.deepcopy(data)
        if self.del_org:
            del results[self.be_copy_key]
        return results
